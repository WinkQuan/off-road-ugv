...
# 计算目标点的吸引势
def attractive_potential(target_location, current_position):
    k_att = 1.0  # Attraction gain
    d_goal_threshold = 2.0  # Goal Threshold
    distance = np.linalg.norm(target_location - current_position)  # 计算当前点到目标点的欧式距离
    if distance <= d_goal_threshold:
        att = k_att * (target_location - current_position)
    else:
        att = (
            d_goal_threshold
            * k_att
            * (target_location - current_position)
            / np.linalg.norm(target_location - current_position)
        )
    return att


# 计算障碍物的排斥势（需要修改）
def repulsive_potential(obs_pos, curr_pos, obs_radius):
...
# 计算加速度
def vel_control(target_location, current_position, obs_pos, mass, obs_radius):
    att = attractive_potential(target_location, current_position)
    rep = repulsive_potential(obs_pos, current_position, obs_radius)
    total_pot = att + rep
